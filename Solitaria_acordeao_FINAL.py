...
while continua == True:
    for i in range(len(baralho)):
        if extrai_naipe(baralho[i]) == "♥" or extrai_naipe(baralho[i]) == "♦":
            print (Fore.RED + "{}.{}".format((i + 1),baralho[i]))
        else:
            print ((Style.RESET_ALL + "{}.{}".format((i + 1),baralho[i])))
...
if len(baralho) == 1:
    ...
    print("Você perdeu, que pena! Para tentar novamente, reinicie o código do arquivo")2